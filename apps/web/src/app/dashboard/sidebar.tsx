...
export default function DashboardSidebar() {
  ...
  return (
    <aside className="h-full w-64 shrink-0 overflow-y-auto border-l border-border bg-muted/20">
      <div className="flex h-full flex-col p-5">
        <div className="mb-4 flex flex-col items-center border-b border-border pb-4 pt-2">
          {logoError ? (
            <div className="flex select-none flex-col items-center leading-tight">
              <span className="text-primary text-2xl font-extrabold tracking-wide">
                Kaffza
              </span>
              <span className="text-sm font-bold tracking-widest text-muted-foreground">قفزة</span>
            </div>
          ) : (
            <img
              src="/logo.png"
          ...
          <span className="bg-primary/10 text-primary mt-2 rounded-full px-3 py-1 text-xs font-semibold">
            {isEn ? 'Merchant' : 'تاجر'}
          </span>
        </div>
        ...
        <div className="rounded-xl bg-card border border-border p-4 shadow-sm mt-4">
          <div className="text-primary text-sm font-bold">{isEn ? 'Note' : 'ملاحظة'}</div>
