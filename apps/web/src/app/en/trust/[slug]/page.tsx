  ...
  return (
    <main dir="ltr" className="mx-auto max-w-4xl px-6 py-12">
      <div className="flex items-center justify-between gap-3">
        <h1 className="text-primary text-3xl font-extrabold">{getTrustTitle(item, true)}</h1>
        <Link className="text-muted-foreground text-sm font-bold underline" href="/en/trust">
          Back to Trust & Safety
        </Link>
      </div>

      <p className="text-foreground/80 mt-3 text-sm">{getTrustSummary(item, true)}</p>

      <Card className="mt-8 p-6">
        <ul className="text-foreground/85 space-y-3 text-sm">
          {getTrustPoints(item, true).map((point) => (
