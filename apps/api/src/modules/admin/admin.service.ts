...
async updateSettings(user: any, body: any) {
  ...
  const next = {
    ...
    escrowNewMerchantDays: body?.escrowNewMerchantDays !== undefined ? Number(body.escrowNewMerchantDays) : current.escrowNewMerchantDays,
    escrowStandardDays: body?.escrowStandardDays !== undefined ? Number(body.escrowStandardDays) : current.escrowStandardDays,
    escrowTrustedDays: body?.escrowTrustedDays !== undefined ? Number(body.escrowTrustedDays) : current.escrowTrustedDays,
    walletMinWithdrawal: body?.walletMinWithdrawal !== undefined ? Number(body.walletMinWithdrawal) : current.walletMinWithdrawal,
    maintenanceMode: body?.maintenanceMode !== undefined ? !!body.maintenanceMode : current.maintenanceMode,
    updatedAt: new Date().toISOString(),
  };

  await this.redis.set(this.settingsKey(), JSON.stringify(next));
  await this.notifications.audit(BigInt(user.sub), 'Admin settings updated', { keys: Object.keys(body || {}) });

  return { success: true, message: 'تم حفظ الإعدادات', data: next };
}
async listUsers(user: any, role?: string) {
  this.assertAdmin(user);
  ...
  for (const u of users as any[]) {
    ...
    result.push({ ...u, status, storesCount: u.role === 'merchant' ? (counts.get(String(u.id)) || 0) : 0 });
  }
...
async setUserSuspended(user: any, userId: bigint, suspended: boolean, reason?: string) {
  ...
  await this.notifications.audit(BigInt(user.sub), `User ${userId.toString()} suspended=${suspended}`, { reason });
  return { success: true };
}
async listMerchants(user: any, status?: string) {
    this.assertAdmin(user);
  ...
  async approveMerchant(user: any, merchantId: bigint, approve: boolean) {
    this.assertAdmin(user);
    const updated = await this.prisma.user.update({ where: { id: merchantId }, data: { isVerified: approve } });
    await this.notifications.audit(BigInt(user.sub), `Merchant ${merchantId.toString()} approval=${approve}`);
  ...
  async setBan(user: any, merchantId: bigint, banned: boolean, reason?: string) {
    ...
    await this.notifications.audit(BigInt(user.sub), `Merchant ${merchantId.toString()} banned=${banned}`, { reason });
    return { success: true };
  }

  async listOrders(user: any) {
    this.assertAdmin(user);
    const orders = await this.prisma.order.findMany({ orderBy: { createdAt: 'desc' }, include: { payment: true, store: { include: { owner: true } }, customer: true } });
    return { success: true, data: orders };
  }

  async listPayments(user: any) {
    this.assertAdmin(user);
    const payments = await this.prisma.payment.findMany({ orderBy: { createdAt: 'desc' }, include: { order: { include: { store: true, customer: true } } } });
    return { success: true, data: payments };
  }

  async listWithdrawals(user: any) {
    this.assertAdmin(user);
    const withdrawals = await this.prisma.withdrawal.findMany({ orderBy: { createdAt: 'desc' }, include: { wallet: { include: { store: true } } } });
  ...
  async handleWithdrawal(user: any, withdrawalId: bigint, approve: boolean, notes?: string) {
    ...
    await this.notifications.audit(BigInt(user.sub), `Withdrawal ${withdrawalId.toString()} approve=${approve}`, { notes });
    return res;
  }
  ...
  async auditLog(user: any) {
    this.assertAdmin(user);
    const logs = await this.prisma.notification.findMany({ where: { type: 'system' }, orderBy: { createdAt: 'desc' }, take: 200 });
