...
listMy(@CurrentUser() user: any, @Query('page') page = '1', @Query('limit') limit = '10') {
  return this.orders.listMyOrdersPaginated(user, Number(page) || 1, Math.min(50, Number(limit) || 10));
}
@Get('orders/my')
  @ApiBearerAuth()
  @UseGuards(JwtAuthGuard)
  ...
  @ApiBearerAuth()
  @UseGuards(JwtAuthGuard)
  updateStatus(@CurrentUser() user: any, @Param('storeId') storeId: string, @Param('orderId') orderId: string, @Body() dto: UpdateStatusDto) {
    return this.orders.updateStatus(user, this.toBigInt(storeId), this.toBigInt(orderId), dto.status);
  }
